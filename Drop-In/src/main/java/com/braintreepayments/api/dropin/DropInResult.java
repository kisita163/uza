...
public class DropInResult implements Parcelable {
    ...
    public static void fetchDropInResult(Activity activity, String clientToken,
        ...
        Log.i("fetchDropInResult","Enter");

        try {
            if (!(Authorization.fromString(clientToken) instanceof ClientToken)) {
                Log.i("fetchDropInResult","Error in Token");
        ...
        Log.i("fetchDropInResult","prepare BraintreeFragment fragment");
        final BraintreeFragment fragment;
        try {
            Log.i("fetchDropInResult","Creating new instance");
        ...
        BraintreeErrorListener errorListener = new BraintreeErrorListener() {
            ...
            public void onError(Exception error) {
                Log.i("fetchDropInResult","Error listener");
                resetListeners(fragment, listenerHolder, previousListeners);
                listener.onError(error);
            }
        };
        listenerHolder.listeners.add(errorListener);
        Log.i("fetchDropInResult","Setting PaymentMethodNoncesUpdatedListener");
        PaymentMethodNoncesUpdatedListener paymentMethodsListener = new PaymentMethodNoncesUpdatedListener() {
            ...
            public void onPaymentMethodNoncesUpdated(java.util.List<PaymentMethodNonce> paymentMethodNonces) {
                resetListeners(fragment, listenerHolder, previousListeners);
                Log.i("fetchDropInResult","Setting PaymentMethodNoncesUpdatedListener --> Update received");
                if (paymentMethodNonces.size() > 0) {
                    Log.i("fetchDropInResult","Setting PaymentMethodNoncesUpdatedListener --> paymentMethodNonces.size() > 0");
        ...
        if (lastUsedPaymentMethodType == PaymentMethodType.ANDROID_PAY || lastUsedPaymentMethodType == PaymentMethodType.GOOGLE_PAYMENT) {
            Log.i("fetchDropInResult","last used payment is " + lastUsedPaymentMethodType);
            ...
            Log.i("fetchDropInResult","Get payment nonce");
            PaymentMethod.getPaymentMethodNonces(fragment);
        }
    }
